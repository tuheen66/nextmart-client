...
export default function AddProductsForm() {
  ...
  //! to create control of form by react-hook form
  const { append: appendFeatures, fields: featureFields } = useFieldArray({
    control: form.control, //* control of form
    name: "keyFeatures", //* control of keyFeature field
  });

  //* set this function the button to add new feature field
  const addFeatures = () => {
  ...
  //! to get all categories and all brands to load when the page loads and then set them in select tags
  useEffect(() => {
    const fetchData = async () => {
      const [categoriesData, brandsData] = await Promise.all([
        getAllCategories(), //* getting all categories from services server function
        getAllBrands(), //* getting all brands from services server function
      ]);

      setCategories(categoriesData?.data); //*setting categories in categories select
      setBrands(brandsData?.data); //*setting brands in brand select
    };
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    //? converting the field value into array of string
    ...
    //? converting the field values into array of object
    ...
    //? adding data as formData to send to backend
                  ...
                  >
                    ...
                    <SelectContent>
                      {categories.map((category) => (
                        //? setting category._id to the value to send to backend and category.name to show in frontend
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  >
                    ...
                    <SelectContent>
                      {brands.map((brand) => (
                        //? setting brand._id to the value to send to backend and brand.name to show in frontend
